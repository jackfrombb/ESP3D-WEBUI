...
function logindlg(closefunc, check_first) {
    ...
    document.getElementById('login_title').innerHTML = translateTextItem("Identification requested");
...
function checkloginsuccess(response_text) {
    ...
    if (typeof(response.authentication_lvl) !== 'undefined') {
        if (response.authentication_lvl != "guest") {
            if (typeof(response.authentication_lvl) !== 'undefined') document.getElementById('current_auth_level').innerHTML = "(" + translateTextItem(response.authentication_lvl) + ")";
...
function loginfailed(errorcode, response_text) {
    var response = JSON.parse(response_text);
    if (typeof(response.status) !== 'undefined') document.getElementById('login_title').innerHTML = translateTextItem(response.status);
    else document.getElementById('login_title').innerHTML = translateTextItem("Identification invalid!");
    console.log("Error " + errorcode + " : " + response_text);
    document.getElementById('login_content').style.display = "block";
    document.getElementById('login_loader').style.display = "none";
    document.getElementById('current_ID').innerHTML = translateTextItem("guest");
