...
function checkpassword() {
    ...
    document.getElementById('password_content').innerHTML = "";
    document.getElementById('change_password_btn').style.display = "none";
    if (pwd1 != pwd2) document.getElementById('password_content').innerHTML = translateTextItem("Passwords do not matches!");
    else if (pwd1.length < 1 || pwd1.length > 16 || pwd1.indexOf(" ") > -1) document.getElementById('password_content').innerHTML = translateTextItem("Password must be >1 and <16 without space!");
...
function ChangePasswordfailed(errorcode, response_text) {
    ...
    if (typeof(response.status) !== 'undefined') document.getElementById('password_content').innerHTML = translateTextItem(response.status);
