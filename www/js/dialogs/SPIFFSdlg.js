...
function SPIFFSdlg(root) {
    ...
    document.getElementById("SPIFFS_file_name").innerHTML = translateTextItem("No file chosen");
...
function closeSPIFFSDialog(msg) {
    if (SPIFFS_upload_ongoing) {
        alertdlg(translateTextItem("Busy..."), translateTextItem("Upload is ongoing, please wait and retry."));
        return;
    }
    closeModal(msg);
}
...
function SPIFFS_Createdir() {
    inputdlg(translateTextItem("Please enter directory name"), translateTextItem("Name:"), processSPIFFS_Createdir);
}
...
function SPIFFSDelete(filename) {
    ...
    confirmdlg(translateTextItem("Please Confirm"), translateTextItem("Confirm deletion of file: ") + filename, processSPIFFSDelete);
}
...
function SPIFFSDeletedir(filename) {
    SPIFFS_currentfile = filename;
    confirmdlg(translateTextItem("Please Confirm"), translateTextItem("Confirm deletion of directory: ") + filename, processSPIFFSDeleteDir);
}
...
function SPIFFSdispatchfilestatus(jsonresponse) {
    var content = "";
    content = translateTextItem("Total:") + " " + jsonresponse.total;
    content += "&nbsp;&nbsp;|&nbsp;&nbsp;" + translateTextItem("Used:") + " " + jsonresponse.used;
    content += "&nbsp;";
    content += "<meter min='0' max='100' high='90' value='" + jsonresponse.occupation + "'></meter>&nbsp;" + jsonresponse.occupation + "%";
    if (jsonresponse.status != "Ok") content += "<br>" + translateTextItem(jsonresponse.status);
    ...
    if (SPIFFS_currentpath != "/") {
        ...
        content += "<tr style='cursor:pointer;' onclick=\"SPIFFS_currentpath='" + previouspath + "'; SPIFFSSendCommand('list','all');\"><td >" + getIconSvg("level-up") + "</td><td colspan='4'> Up..</td></tr>";
    }
    ...
    for (var i = 0; i < jsonresponse.files.length; i++) {
        ...
        if (String(jsonresponse.files[i].size) != "-1") {
            content += "<tr>";
            content += "<td  style='vertical-align:middle; color:#5BC0DE'>" + getIconSvg("file") + "</td>";
    ...
    for (var i = 0; i < jsonresponse.files.length; i++) {
        if (String(jsonresponse.files[i].size) == "-1") {
            content += "<tr>";
            content += "<td style='vertical-align:middle ; color:#5BC0DE'>" + getIconSvg("folder-close") + "</td>";
...
function SPIFFSUploadProgressDisplay(oEvent) {
    if (oEvent.lengthComputable) {
        ...
        document.getElementById('uploadSPIFFSmsg').innerHTML = translateTextItem("Uploading ") + SPIFFS_currentfile + " " + percentComplete.toFixed(0) + "%";
    } else {
...
function SPIFFS_UploadFile() {
    ...
    document.getElementById('uploadSPIFFSmsg').innerHTML = translateTextItem("Uploading ") + SPIFFS_currentfile;
...
function SPIFFSUploadfailed(errorcode, response) {
    ...
    if (esp_error_code !=0){
         ...
         document.getElementById('SPIFFS_status').innerHTML = translateTextItem("Error : ") + esp_error_message;
         esp_error_code = 0;
    } else {
        alertdlg (translateTextItem("Error"), "Error " + errorcode + " : " + response);
        document.getElementById('SPIFFS_status').innerHTML = translateTextItem("Upload failed : ") + errorcode;
    }
    SPIFFS_upload_ongoing = false;
    refreshSPIFFS();
}
