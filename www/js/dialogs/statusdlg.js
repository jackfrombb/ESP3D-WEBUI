...
function update_btn_status(forcevalue) {
    ...
    if (statuspage == 0) {
        statuspage = 1;
        document.getElementById('next_status_btn').innerHTML = getIconSvg("triangle-right", "1em", "1em")
    } else {
        statuspage = 0;
        document.getElementById('next_status_btn').innerHTML = getIconSvg("triangle-left", "1em", "1em")
    }
}
...
function statussuccess(response) {
    ...
    for (var i = 0; i < tresponse.length; i++) {
        ...
        if (data.length >= 2) {
            statuscontent += "<label>" + translateTextItem(data[0]) + ": </label>&nbsp;<span class='text-info'><strong>";
            var data2 = data[1].split(" (")
            statuscontent += translateTextItem(data2[0].trim());
    ...
    statuscontent += "<label>" + translateTextItem("WebUI version") + ": </label>&nbsp;<span class='text-info'><strong>";
