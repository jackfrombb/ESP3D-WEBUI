...
function buildLanguageList(id_item) {
    var content = "<select class='form-control'  id='" + id_item + "' onchange='translateText(this.value)'>\n";
...
function translateText(lang) {
    ...
    for (var i = 0; i < All.length; i++) {
        if (All[i].hasAttribute('translate')) {
            ...
            translated_content = translateTextItem(content);
        ...
        if (All[i].hasAttribute('translateph') && All[i].hasAttribute('placeholder')) {
            ...
            translated_content = decodeEntities(translateTextItem(content));
            All[i].setAttribute('placeholder', translated_content)
        }
    }
};

function translateTextItem(item_text, withtag) {
    var currenttrans = {};
    var translated_content;
    var with_tag = false;
