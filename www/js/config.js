...
function refreshConfig(is_override) {
    ...
    if (http_communication_locked) {
        document.getElementById('config_status').innerHTML =
            translateTextItem("Communication locked by another process, retry later.");
        return;
    }
    ...
    if (target_firmware === "smoothieware")
        response_text =
            ...
            "# The maximum error for line segments that divide arcs 0 to disable" +
            "\n                                                              " +
...
function build_HTML_config_list() {
    ...
    for (var i = 0; i < array_len; i++) {
        ...
        if (item.showcomment) {
            ...
            content += "<input class='hide_it'></input>";
            content += "<span class='input-group-btn'>";
            content += "<button  id='btn_config_" + prefix + i + "' class='btn btn-default' onclick='configGetvalue(" + i + "," + is_override_config + ")' translate english_content='Set' >" + translateTextItem("Set") + "</button>&nbsp;";
            content += "</span>";
            content += "</div>";
            content += "</div>";
            content += "</div>";
...
function config_check_value(value, index, is_override) {
    ...
    if ((target_firmware == "smoothieware") && !is_override) {
        if ((value.trim()[0] == '-') || (value.length === 0) || (value.toLowerCase().indexOf("#") != -1)) {
            isvalid = false;
            config_error_msg = translateTextItem("cannot have '-', '#' char or be empty");
        }
    }
    if ((target_firmware == "grbl") || (target_firmware == "grbl-embedded")) {
        ...
        if ((value.trim()[0] == '-') || (value.length === 0) || ((value.indexOf("e") != -1) && (value.toLowerCase().indexOf("true") == -1) && (value.toLowerCase().indexOf("false") == -1))) {
            isvalid = false;
            config_error_msg = translateTextItem("cannot have '-', 'e' char or be empty");
        }
    }
    return isvalid;
}
...
function process_config_answer(response_text) {
    ...
    if (tlines.length <= 3) {
        if ((target_firmware == "smoothieware") && (commandtxt != "cat /sd/config.txt")) {
            ...
            if ((target_firmware == "smoothieware")) document.getElementById('config_status').innerHTML = translateTextItem("File config / config.txt not found!");
            else document.getElementById('config_status').innerHTML = translateTextItem("Cannot get EEPROM content!");
            result = false;
        }
    } else {
...
function configGetvalue(index, is_override) {
    ...
    if (!isvalid) {
        ...
        document.getElementById('icon_config_' + prefix + index).innerHTML = getIconSvg("remove");
        document.getElementById('status_config_' + prefix + index).className = "form-group has-feedback has-error";
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value ") + config_error_msg + " !");
    } else {
        //value is ok save it
        ...
        document.getElementById('icon_config_' + prefix + index).innerHTML = getIconSvg("ok");
...
function config_checkchange(index, is_override) {
    ...
    if (item.defaultvalue == val) {
        ...
        document.getElementById('icon_config_' + prefix + index).innerHTML = getIconSvg("warning-sign");
...
function inline_help(label) {
    ...
    return translateTextItem(shelp);
}

function setESPconfigfailed(error_code, response) {
    alertdlg(translateTextItem("Set failed"), "Error " + error_code + " :" + response);
    ...
    document.getElementById('icon_config_' + prefix + config_lastindex).innerHTML = getIconSvg("remove");
...
function getESPconfigSuccess(response) {
    ...
    if (!process_config_answer(response)) {
        getESPconfigfailed(406, translateTextItem("Wrong data"));
        document.getElementById('config_loader').style.display = "none";
        document.getElementById('config_list_content').style.display = "block";
        document.getElementById('config_status').style.display = "none";
        document.getElementById('config_refresh_btn').style.display = "block";
...
function getESPconfigfailed(error_code, response) {
    ...
    document.getElementById('config_status').innerHTML = translateTextItem("Failed:") + error_code + " " + response;
    document.getElementById('config_refresh_btn').style.display = "block";
}
