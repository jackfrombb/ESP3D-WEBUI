...
function files_set_button_as_filter(isfilter) {
    if (!isfilter) {
        document.getElementById('files_filter_glyph').innerHTML = getIconSvg("filter", "1em", "1em");
    } else {
        document.getElementById('files_filter_glyph').innerHTML = getIconSvg("list-alt", "1em", "1em");
    }
}
...
function files_build_file_line(index) {
    ...
    if ((files_filter_sd_list && entry.isprintable) || (!files_filter_sd_list)) {
        ...
        if (entry.isdir == true) content += getIconSvg("folder-open");
        else content += getIconSvg("file");
        content += "</span ></td><td>";
        ...
        if (entry.isprintable) {
            ...
            if ((target_firmware == "grbl-embedded") || (target_firmware == "grbl")) content += getIconSvg("play", "1em", "1em");
            else content += getIconSvg("print", "1em", "1em");
            content += "</button>";
        }
        content += "&nbsp;";
        if (files_showdeletebutton(index)) {
            content += "<button class='btn btn-xs btn-danger' onclick='files_delete(" + index + ")'  style='padding-top: 4px;'>" + getIconSvg("trash", "1em", "1em") + "</button>";
        }
        content += "</div>";
        content += "</div>";
        content += "</div>";
...
function files_Createdir() {
    inputdlg(translateTextItem("Please enter directory name"), translateTextItem("Name:"), process_files_Createdir);
}
...
function files_delete(index) {
    ...
    var msg = translateTextItem("Confirm deletion of directory: ");
    if (!files_file_list[index].isdir) msg = translateTextItem("Confirm deletion of file: ");
    confirmdlg(translateTextItem("Please Confirm"), msg + files_file_list[index].name, process_files_Delete);
}
    ...
    if (answer.startsWith("{") && answer.endsWith("}")) {
        try {
            ...
            if (typeof response.status != 'undefined') {
                monitor_outputUpdate(response.status + "\n");
        ...
        if (answer[answer.length - 1] != '\n') monitor_outputUpdate(answer + "\n");
        else monitor_outputUpdate(answer);
    ...
    if (direct_sd && !(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
        ...
        if (target_firmware == "smoothieware") {
            ...
            if (current_source == "SD:") {
                ...
                //Standard M20
                current_source = "/";
                document.getElementById('fileSource').innerHTML = translateTextItem("SD Files");
            }
...
function files_directSD_list_success(response_text) {
    ...
    if (error || typeof response.status == 'undefined') {
        files_directSD_list_failed(406, translateTextItem("Wrong data", true));
        return;
    }
    files_file_list = [];
    files_status_list = [];
    ...
    files_status_list.push({
        status: translateTextItem(response.status),
        path: response.path,
        used: vused,
        total: vtotal,
...
function files_serial_M20_list_failed(error_code, response) {
    ...
    if (esp_error_code != 0) {
        ...
        alertdlg(translateTextItem("Error"), translateTextItem("No connection"));
    }
    files_build_display_filelist(false);
}
...
function files_build_display_filelist(displaylist) {
    ...
    if (need_up_level()) {
        ...
        content += "<span >" + getIconSvg("level-up") + "</span>&nbsp;&nbsp;<span translate>Up...</span>";
        content += "</li>";
    }
    ...
    if (files_status_list.length > 0) {
        ...
        if (files_status_list[0].status.toLowerCase() != "ok") {
            document.getElementById('files_sd_status_msg').innerHTML = translateTextItem(files_status_list[0].status, true);
...
function check_for_8_3(files) {
    for (var i = 0; i < files.length; i++) {
        ...
        if (filename.indexOf(".") != filename.lastIndexOf(".")
            || filename.indexOf(".") > 8
            || filename.split(".")[1].length > 4) {

            //Change name
            var newName = filename.slice(0, filename.indexOf(".") > 8 ? 8 : filename.indexOf("."));

            //Change ext
            var newExt = filename.slice(filename.lastIndexOf("."), filename.length);
            if(newExt.length > 4) newExt = newExt.slice(0, 4);

            //Show alert
            alertdlg(translateTextItem("Attention"),
                translateTextItem("File renamed with 8.3 filename convention: ") + (newName + newExt));

            //Rename file
            Object.defineProperty(files[i], 'name', {
                writable: true,
                value: (newName + newExt)
            });
        }
    }
...
function process_check_sd_presence(answer) {
    ...
    if (direct_sd && !(target_firmware == "smoothieware" && files_currentPath.startsWith(secondary_sd))) {
        if (answer.indexOf("o SD card") > -1) {
            alertdlg(translateTextItem("Upload failed"), translateTextItem("No SD card detected"));
            files_error_status = "No SD card"
            files_build_display_filelist(false);
            document.getElementById('files_sd_status_msg').innerHTML = translateTextItem(files_error_status, true);
            document.getElementById('files_status_sd_status').style.display = "table-row";
