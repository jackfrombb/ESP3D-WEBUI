...
function Extrude_cmd(extruder, direction) {
    ...
    if (velocity < 1 || velocity > 9999 || isNaN(velocity) || (velocity === null)) {
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value of extruder velocity must be between 1 mm/min and 9999 mm/min !"));
        return;
    }
    if (filament_length < 0.001 || filament_length > 9999 || isNaN(filament_length) || (filament_length === null)) {
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value of filament length must be between 0.001 mm and 9999 mm !"));
        return;
    }
...
function flowSet_cmd() {
    ...
    if (value < 50 || value > 300 || isNaN(value)) {
        document.getElementById('flowSelectedValue').value = 100;
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value must be between 50% and 300% !"));
    } else {
...
function feedSet_cmd() {
    ...
    if (value < 25 || value > 150 || isNaN(value)) {
        document.getElementById('feedSelectedValue').value = 100;
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value must be between 25% and 150% !"));
    } else {
        sendPrinterCommand(command + value, true);
    }
}
...
function fanSet_cmd() {
    ...
    if (fvalue < 0 || fvalue > 100 || isNaN(fvalue) || fvalue === null) {
        document.getElementById('fanSelectedValue').value = 0;
        alertdlg(translateTextItem("Out of range"), translateTextItem("Value must be between 0% and 100% !"));
    } else {
        sendPrinterCommand(command + value, true);
    }
}
